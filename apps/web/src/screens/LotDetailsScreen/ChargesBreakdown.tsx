  ...
  if (charges.length === 0) {
    return (
      <div className="flex flex-col items-center gap-1 px-4 py-10 text-center">
        <p className="text-base font-medium text-text-primary">{t('lot_detail.charges_empty_title')}</p>
        <p className="text-sm text-text-tertiary">{t('lot_detail.charges_empty_body')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="px-4 pb-6 pt-2">
      ...
      <ul className="mt-3 flex flex-col gap-3">
        {charges.map((c) => {
          ...
          return (
            <li
              key={c.id}
              className="flex flex-col gap-1 border-b border-border pb-3 last:border-0 sm:flex-row sm:items-center sm:justify-between sm:gap-4"
            >
              <span className="text-sm text-text-primary">{c.charge_type_label}</span>
              <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                <span className="text-sm font-semibold text-text-primary">{formatINR(amt)}</span>
                <span
                  className={cn(
                    'text-xs font-medium',
                    c.is_paid ? 'text-[#0B7B6E]' : 'text-[#A83422]',
                  )}
                >
      ...
      <div className="mt-4 border-t border-border pt-4">
        ...
        <div className="mt-2 flex justify-between text-base font-semibold text-text-primary">
          <span>{t('lot_detail.collected')}</span>
          <span className="text-[#0B7B6E]">{formatINR(collected)}</span>
        </div>
        <div className="mt-2 flex justify-between text-base font-semibold text-text-primary">
          <span>{t('lot_detail.pending')}</span>
          <span className="text-[#A83422]">{formatINR(pending)}</span>
        </div>
      </div>
    </div>
  );
