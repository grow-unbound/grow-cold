...
/** Insert lot (RLS must allow; mirrors web POST /api/lots body). Returns new lot id. */
export async function insertWarehouseLot(client: SB, input: CreateLotInput): Promise<string> {
  ...
  if (!row?.id) {
    throw new Error('Lot insert did not return id');
  }
  return row.id;
}
