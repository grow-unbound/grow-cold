...
| Lot Number | Text, auto-suggested | Required; unique across all lots; suggest as `[PartyCode]-[incrementing 3-digit number]`; user may override |
| Locations | Multiselect from locations table | Required; at least 1 location; e.g. "Row B Rack 3", "Cold Room 1" |
...
- **Edit mode**: Lot Number locked by default; unlock requires inline confirmation — "Changing lot number affects all linked records. Confirm?"
- **Edit mode**: Party and Product are read-only if lot has any inward or delivery records; show lock icon with "Cannot change — lot has movements"
...
4. Lot Number: text input. Auto-suggested based on last lot number for the selected party (increment sequence). Show "suggested" label when auto-filled; label clears when user edits. Required. Async uniqueness check on blur — show "Lot number already exists" if taken.
...
```
Party:    RC01 — Raju Cold Store    [locked]
Lot:      GC-RC01-047               [locked]
Movement: Lodgement ▼
```
Party and Lot are always auto-filled from context and locked. Movement defaults based on entry point; user can change if the lot has multiple movements.
...
Pre-load the following charge types in this order:
1. Hamali
2. Mamulle
3. Kata-coolie
4. Transport
...
- Applicable Bags: numeric integer input
  ...
  - Kata-coolie: pre-filled with 0; label is "Weighed Bags" (not "Bags")
...
- Pre-loaded charge type rows: Hamali, Mamulle, Kata-coolie, Transport
...
| Lot Reference | Selector | Link to a specific lot |
| Delivery Reference | Selector | Link to a delivery; filtered by Lot if Lot is set |
| Notes | Multiline text | — |
...
- If party has no outstanding accruals: show empty state "No outstanding dues. Full ₹[amount] saved as credit." with Confirm button that just saves unapplied credit.
