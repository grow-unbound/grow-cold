...
export async function GET(request: Request) {
  ...
  try {
    const body = await fetchBusinessSnapshot(auth.supabase, auth.data.warehouseId, new Date());
    return NextResponse.json(CommandCenterSnapshotResponseSchema.parse(body));
  } catch (e) {
    console.error(e);
