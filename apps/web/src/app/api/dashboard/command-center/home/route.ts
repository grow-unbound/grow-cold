...
export async function GET(request: Request) {
  ...
  try {
    const body = await fetchCommandCenterHome(auth.supabase, auth.data.warehouseId, new Date());
    return NextResponse.json(CommandCenterHomeResponseSchema.parse(body));
  } catch (e) {
    console.error(e);
